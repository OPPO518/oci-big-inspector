  ...
  <div v-if="!loading && needInit" class="init-pure-wrapper">
    <div class="init-card fade-in">
      ...
      <form @submit.prevent="submitInit">
        <div class="form-group"><label>管理员账号</label><input v-model="initForm.username" type="text" required placeholder="请输入管理员用户名..." /></div>
        <div class="form-group"><label>管理员密码</label><input v-model="initForm.password" type="password" required placeholder="请输入安全访问密码..." /></div>
  ...
  <div v-else-if="!loading && !needInit && !isLoggedIn" class="init-pure-wrapper">
    <div class="init-card fade-in">
      ...
      <form @submit.prevent="submitLogin">
        <div class="form-group"><label>管理员账号</label><input v-model="loginForm.username" type="text" required placeholder="请输入用户名" /></div>
        <div class="form-group"><label>管理员密码</label><input v-model="loginForm.password" type="password" required placeholder="请输入密码" /></div>
        <button type="submit" :disabled="submitting" class="btn btn-check" style="width:100%; justify-content: center;">
          登 录
        </button>
      </form>
  ...
  <div v-else-if="isLoggedIn" class="app-layout fade-in">
    <aside class="sidebar">
      ...
      <div class="sidebar-menu">
        ...
        <a href="#" class="menu-item" :class="{ active: currentTab === 'quota' }" @click="currentTab = 'quota'"><i class="fa-solid fa-earth-americas"></i> OCI 区域管理</a>

        <div class="menu-group">群控自动化</div>
        <a href="#" class="menu-item" :class="{ active: currentTab === 'boot' }" @click="currentTab = 'boot'"><i class="fa-solid fa-circle-play"></i> OCI 开机管理</a>
        <a href="#" class="menu-item" :class="{ active: currentTab === 'boot-logs' }" @click="currentTab = 'boot-logs'"><i class="fa-solid fa-terminal"></i> OCI 开机日志</a>

        <div class="menu-group">资源 management</div>
        <a href="#" class="menu-item" :class="{ active: currentTab === 'instances' }" @click="currentTab = 'instances'"><i class="fa-solid fa-server"></i> OCI 实例列表</a>

        <div class="menu-group">系统管理</div>
        <a href="#" class="menu-item" :class="{ active: currentTab === 'security' }" @click="currentTab = 'security'"><i class="fa-solid fa-user-shield"></i> 安全管理</a>
      ...
      <main class="main-content">
            ...
            <div class="m-box">
              ...
              <div class="box-body split-layout">
                <div class="circle-chart">
                  <svg viewBox="0 0 36 36" class="circular-chart blue-ring"><path class="circle-bg" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" /><path class="circle" :style="{ strokeDasharray: monitorData.cpu_usage + ', 100' }" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" /><text x="18" y="20.3" class="percentage font-mono">{{ monitorData.cpu_usage }}%</text></svg>
                </div>
                <div class="details-list font-mono"><div class="item"><span class="lbl">物理核心:</span><span class="val">1 C</span></div><div class="item"><span class="lbl">逻辑核心:</span><span class="val">2 C</span></div><div class="item"><span class="lbl">架构型号:</span><span class="val truncated" :title="monitorData.cpu_model">{{ monitorData.cpu_model }}</span></div></div>
              </div>
            </div>
            <div class="m-box">
              ...
              <div class="box-body split-layout">
                <div class="circle-chart">
                  <svg viewBox="0 0 36 36" class="circular-chart green-ring"><path class="circle-bg" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" /><path class="circle" :style="{ strokeDasharray: monitorData.mem_usage_pct + ', 100' }" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" /><text x="18" y="20.3" class="percentage font-mono">{{ monitorData.mem_usage_pct }}%</text></svg>
                </div>
                <div class="details-list font-mono"><div class="item"><span class="lbl">总内存:</span><span class="val">{{ monitorData.mem_total ? monitorData.mem_total.toFixed(2) : 3.83 }} GB</span></div><div class="item"><span class="lbl">已用内存:</span><span class="val">{{ monitorData.mem_used ? monitorData.mem_used.toFixed(2) : 0.57 }} GB</span></div></div>
              </div>
            </div>
            <div class="m-box">
              ...
              <div class="box-body split-layout">
                <div class="circle-chart">
                  <svg viewBox="0 0 36 36" class="circular-chart orange-ring"><path class="circle-bg" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" /><path class="circle" style="stroke-dasharray: 100, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" /><text x="18" y="20.3" class="percentage text-sm">运行中</text></svg>
                </div>
                <div class="details-list font-mono"><div class="item"><span class="lbl">操作系统:</span><span class="val">{{ monitorData.os_info || 'Linux' }}</span></div><div class="item"><span class="lbl">运行时间:</span><span class="val text-primary">{{ monitorData.uptime }}</span></div></div>
              </div>
            </div>
          </div>
          <div class="monitor-grid-bottom">
            <div class="m-box-full">
              ...
              <div class="storage-bar-area font-mono">
                <div class="storage-info"><span>挂载路径：<b>/app/data (SQLite 数据落盘池)</b></span><span>已用：{{ monitorData.disk_used ? monitorData.disk_used.toFixed(2) : 5.52 }} GB / 总容量：{{ monitorData.disk_total ? monitorData.disk_total.toFixed(2) : 9.65 }} GB</span></div>
        ...
        <div v-else-if="currentTab === 'tenant'">
          <header class="dash-header">
            ...
            <div class="search-bar"><input v-model="searchQuery" type="text" placeholder="输入自定义名称或区域过滤..." /><button class="btn-search"><i class="fa-solid fa-magnifying-glass"></i></button></div>
            <div class="btn-group">
          ...
          <div class="table-container">
            <table>
              <thead>
                <tr>
                  <th>#</th><th>自定义名称</th><th>租户名</th><th>账号类型</th><th>区域</th><th>是否多区</th><th>创建时间</th><th>存活天数</th><th>开机任务</th><th>账号状态</th><th>专属代理</th><th>操作</th>
                </tr>
              </thead>
              <tbody>
                <tr v-if="filteredAccounts.length === 0"><td colspan="12" class="text-center" style="padding: 40px; color: #4b5563;">暂无凭证</td></tr>
                <tr v-for="(acc, index) in filteredAccounts" :key="acc.id">
                  ...
                  <td class="action-cell">
                    <button class="btn-create-spec"><i class="fa-solid fa-rocket"></i> 创建</button>
        ...
        <div v-else-if="currentTab === 'security'" class="placeholder-container card" style="text-align: left; max-width: 700px; padding: 40px;">
          ...
          <form @submit.prevent="saveTgConfig">
            ...
            <div class="form-group" style="display: flex; align-items: center; margin-top: 20px;"><label style="margin-bottom: 0; margin-right: 15px;">是否开启通知</label><input v-model="tgForm.tg_notify_enabled" type="checkbox" true-value="1" false-value="0" style="width: 20px; height: 20px;" /></div>
        ...
        <div class="placeholder-container card" v-else>
          ...
          <h3>「 核心模块：{{ currentTab.toUpperCase() }} 」已卡位</h3>
        </div>
      </main>
      ...
      <div v-if="showModal" class="modal-overlay" @click.self="showModal = false">
        <div class="modal-content fade-in-up">
          ...
          <form @submit.prevent="submitAddAccount">
            <div class="form-group"><label>1. OCI 原始凭证 (Config)</label><textarea v-model="addForm.raw_config" rows="4" class="code-input"></textarea></div>
            <div class="form-group"><label>2. 自定义名称</label><input v-model="addForm.alias" type="text" required /></div>
            <div class="form-group"><label>3. 专属代理网络</label><input v-model="addForm.proxy" type="text" placeholder="IP:PORT，直连请留空" /></div>
            <div class="form-group">
              <label>4. 密钥文件</label>
              <div class="file-upload-wrapper">
                <input type="file" @change="handleFileUpload" accept=".pem,.key" id="file-upload" class="hidden-file-input" />
                <label for="file-upload" class="file-upload-btn"><i class="fa-solid fa-file-shield"></i> 选择私钥</label>
                <span class="text-sm font-mono" style="margin-left: 10px; color: #38bdf8;">{{ uploadedFileName }}</span>
              </div>
              <textarea v-model="addForm.private_key" rows="3" placeholder="或在此粘贴 Private Key" style="margin-top:10px; font-family: monospace;"></textarea>
            </div>
...
const searchQuery = ref(''); const uploadedFileName = ref('未选择文件')
...
axios.interceptors.response.use(response => response, error => {
  if (error.response && error.response.status === 401) { isLoggedIn.value = false; localStorage.removeItem('big_inspector_auth'); delete axios.defaults.headers.common['Authorization'] }
...
const runSingleAccountTest = async (acc) => {
  ...
  try {
    ...
    if (testRes.data && testRes.data.status === 'success') {
      ...
      if (acc.created_at) { const t = new Date(acc.created_at.replace(' ', 'T')); acc.alive_days = Math.floor((new Date() - t) / (1000 * 60 * 60 * 24)) <= 0 ? 1 : Math.floor((new Date() - t) / (1000 * 60 * 60 * 24)) }
...
watch(currentTab, (newTab) => { if (needInit.value || !isLoggedIn.value) return; if (newTab === 'monitor') fetchMonitorData(); if (newTab === 'security') fetchTgConfig(); if (newTab === 'tenant') fetchAccounts() })
...
const filteredAccounts = computed(() => { if (!searchQuery.value) return accounts.value; return accounts.value.filter(acc => acc.alias.toLowerCase().includes(searchQuery.value.toLowerCase()) || acc.region.toLowerCase().includes(searchQuery.value.toLowerCase())) })
const handleFileUpload = (event) => { const file = event.target.files[0]; if (!file) return; uploadedFileName.value = file.name; const reader = new FileReader(); reader.onload = (e) => { addForm.value.private_key = e.target.result }; reader.readAsText(file) }
...
.mini-icon.blue { background: rgba(56,189,248,0.1); color: #38bdf8; } .mini-icon.green { background: rgba(16,185,129,0.1); color: #10b981; } .mini-icon.orange { background: rgba(245,158,11,0.1); color: #f59e0b; } .mini-icon.success { background: rgba(16,185,129,0.15); color: #34d399; } .mini-icon.danger { background: rgba(239,68,68,0.1); color: #ef4444; }
...
.m-box { background: #111827; border: 1px solid #1f2937; border-radius: 8px; padding: 20px; } .m-box-full { background: #111827; border: 1px solid #1f2937; border-radius: 8px; padding: 20px; margin-bottom: 25px; }
...
.circle-chart { width: 100px; height: 100px; position: relative; } .circular-chart { display: block; max-height: 100px; } .circle-bg { stroke: #1f2937; stroke-width: 2.8; fill: none; } .circle { stroke-width: 2.8; stroke-linecap: round; fill: none; transition: stroke-dasharray 0.4s ease; } .blue-ring .circle { stroke: #38bdf8; } .green-ring .circle { stroke: #10b981; } .orange-ring .circle { stroke: #f59e0b; } .percentage { fill: #fff; font-size: 8px; text-anchor: middle; font-weight: bold; } .percentage.text-sm { font-size: 5px; fill: #9ca3af; }
.details-list { flex: 1; display: flex; flex-direction: column; gap: 8px; font-size: 12px; } .details-list .item { display: flex; justify-content: space-between; border-bottom: 1px dashed #1f2937; padding-bottom: 4px; } .details-list .lbl { color: #64748b; } .details-list .val { color: #f8fafc; font-weight: 500; } .truncated { max-width: 120px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: inline-block; }
.storage-bar-area { display: flex; flex-direction: column; gap: 10px; } .storage-info { display: flex; justify-content: space-between; font-size: 13px; color: #9ca3af; } .progress-container-bar { background: #1f2937; height: 10px; border-radius: 5px; overflow: hidden; } .progress-fill-bar { background: linear-gradient(90deg, #10b981, #34d399); height: 100%; width: 0; transition: width 0.5s ease; }
...
.search-bar { display: flex; width: 320px; } .search-bar input { flex: 1; background: #0b0f19; border: 1px solid #1f2937; padding: 8px 12px; border-radius: 6px 0 0 6px; color: #fff; outline: none; font-size: 13px; } .btn-search { background: #2563eb; border: none; color: white; padding: 0 14px; border-radius: 0 6px 6px 0; cursor: pointer; }
...
.badge { padding: 3px 6px; border-radius: 4px; font-size: 11px; font-weight: 600; } .badge-info { background: rgba(56,189,248,0.1); color: #38bdf8; } .badge-success { background: rgba(16,185,129,0.1); color: #10b981; } .badge-danger { background: rgba(239,68,68,0.1); color: #ef4444; } .badge-neutral { background: #1f2937; color: #9ca3af; border: 1px solid #374151; } .badge-warning { background: rgba(245,158,11,0.1); color: #f59e0b; }
...
.hidden-file-input { display: none; } .file-upload-btn { background: #1f2937; color: #e5e7eb; padding: 8px 14px; border-radius: 6px; cursor: pointer; font-size: 12px; border: 1px solid #374151; }
